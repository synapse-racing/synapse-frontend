...
export function createRoadGeometry(track: TrackDefinition) {
  if (track.geometry.kind === 'rectangular-ring') {
    ...
    const shape = new Shape().moveTo(-outerX, -outerZ).lineTo(outerX, -outerZ).lineTo(outerX, outerZ).lineTo(-outerX, outerZ).closePath()
    shape.holes.push(new Path().moveTo(-innerX, -innerZ).lineTo(-innerX, innerZ).lineTo(innerX, innerZ).lineTo(innerX, -innerZ).closePath())
  ...
  for (let i = 0; i < count; i++) {
    const left = track.boundaries[i * 2], right = track.boundaries[i * 2 + 1]
...
export function trackExtent(track: TrackDefinition) {
  return {
    x: Math.max(...track.boundaries.flatMap(b => [Math.abs(b[0]), Math.abs(b[2])])),
    z: Math.max(...track.boundaries.flatMap(b => [Math.abs(b[1]), Math.abs(b[3])])),
  }
}
