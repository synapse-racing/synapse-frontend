...
describe.each(['technical-loop-v2', 'grand-prix-v3'] as const)(
  '%s circuits',
  (version) => {
    it('allows a steering controller to complete full laps with the real car physics', () => {
      for (const seed of [0, 1, 2, 7, 42, 123, 42170, 999999]) {
        const track = generateTrack({ version, seed })
        if (track.geometry.kind !== 'centerline-loop')
        ...
        for (let step = 0; step < 3600; step++) {
          ...
          const target =
            points[
              (nearest + (version === 'grand-prix-v3' ? 3 : 2)) % points.length
            ]
          const yaw = Math.atan2(-(target[0] - state.x), -(target[1] - state.z))
          const error = Math.atan2(
            Math.sin(yaw - state.yaw),
            Math.cos(yaw - state.yaw),
          )
          const result = stepSimulation(
            state,
            Math.max(-1, Math.min(1, error * 1.8)),
            0.65,
            track,
          )
    ...
    it('generates long closed circuits with alternating bends and non-crossing road edges', () => {
      for (let seed = 0; seed < 40; seed++) {
        ...
        expect([finish.position[0], finish.position[2]]).toEqual([
        ...
        for (let i = 0; i < track.boundaries.length; i++) {
          ...
          for (let j = i + 1; j < track.boundaries.length; j++) {
            ...
            const crosses =
              side(cx, cz, ax, az, bx, bz) * side(dx, dz, ax, az, bx, bz) <
                -1e-8 &&
              side(ax, az, cx, cz, dx, dz) * side(bx, bz, cx, cz, dx, dz) <
                -1e-8
            if (crosses)
              throw new Error(
                `Crossing road edges for seed ${seed}: ${i}, ${j}`,
              )
          }
        }
      }
    ...
    it('requires every sector before the shared start/finish counts as a lap', () => {
      const track = generateTrack({ version, seed: 42170 })
      const state = createSimulationState(track)
